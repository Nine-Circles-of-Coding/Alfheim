...
public class WE_CaveGen extends WE_CreateChunkGen {
    ...
    public void gen(WE_GeneratorData data) {
        ...
        for (long cx = data.chunk_X / 16L - (long) range; cx <= data.chunk_X / 16L + (long) range; ++cx)
            for (long cz = data.chunk_Z / 16L - (long) range; cz <= data.chunk_Z / 16L + (long) range; ++cz) {
                ...
                if (rand.nextInt(7) != 0) e = 0;
                // -//
                for (int i = 0; i < e; ++i) {
                    double gx = (double) cx * 16.0D + (double) rand.nextInt(16),
                        gy = rand.nextInt(rand.nextInt(120) + 8), gz = (double) cz * 16.0D + (double) rand.nextInt(16);
                    ...
                    for (int l1 = 0; l1 < k; ++l1) {
                        float x = rand.nextFloat() * 2.0F * (float) Math.PI, x1 = (rand.nextFloat() - 0.5F) * 0.25F,
                            x2 = rand.nextFloat() * 2.0F + rand.nextFloat();
                        if (rand.nextInt(10) == 0) x2 *= rand.nextFloat() * rand.nextFloat() * 3.0F + 1.0F;
    ...
    public void caveGen_func(WE_GeneratorData data, long rs, double gx, double gy, double gz, float fn1, float fn2,
        float fn3, int in1, int in2, double dn) {
        ...
        for (boolean flag = r.nextInt(6) == 0; in1 < in2; ++in1) {
            double d6 = 1.5D + (double) MathHelper.sin((float) in1 * (float) Math.PI / (float) in2) * (double) fn1,
                d7 = d6 * dn;
            ...
            if (!flag2 && in1 == k1 && fn1 > 1.0F && in2 > 0) {
                caveGen_func(
                    ...
                    gy,
                    gz,
                    r.nextFloat() * 0.5F + 0.5F,
                    fn2 - (float) Math.PI / 2.0F,
                    fn3 / 3.0F,
                    in1,
                    in2,
                    1.0D);
            ...
            if (flag2 || r.nextInt(4) != 0) {
                ...
                if (gx >= cx - 16.0D - d6 * 2.0D && gz >= cz - 16.0D - d6 * 2.0D
                    && gx <= cx + 16.0D + d6 * 2.0D
                    && gz <= cz + 16.0D + d6 * 2.0D) {
                    int i4 = MathHelper.floor_double(gx - d6) - (int) data.chunk_X - 1,
                        l1 = MathHelper.floor_double(gx + d6) - (int) data.chunk_X + 1,
                        j4 = MathHelper.floor_double(gy - d7) - 1, i2 = MathHelper.floor_double(gy + d7) + 1,
                        k4 = MathHelper.floor_double(gz - d6) - (int) data.chunk_Z - 1,
                        j2 = MathHelper.floor_double(gz + d6) - (int) data.chunk_Z + 1;
                    ...
                    for (k2 = i4; !flag3 && k2 < l1; ++k2)
                        for (int l2 = k4; !flag3 && l2 < j2; ++l2) for (int i3 = i2 + 1; !flag3 && i3 >= j4 - 1; --i3) {
                            j3 = (k2 * 16 + l2) * 256 + i3;
                            if (i3 >= 0 && i3 < 256) {
                                if (isOceanBlock(data, j3, k2, i3, l2)) flag3 = true;
                                if (i3 != j4 - 1 && k2 != i4 && k2 != l1 - 1 && l2 != k4 && l2 != j2 - 1) i3 = j4;
                            }
                        }
                    // -//
                    if (!flag3) {
                        for (k2 = i4; k2 < l1; ++k2) {
                            ...
                            for (j3 = k4; j3 < j2; ++j3) {
                                ...
                                if (d13 * d13 + d14 * d14 < 1.0D) for (int l3 = i2 - 1; l3 >= j4; --l3) {
    ...
    public void digBlock(WE_GeneratorData data, int index, int x, int y, int z) {
        ...
        for (int i = 0; i < replaceBlocksList.size(); i++) if (data.chunkBlocks[index] == replaceBlocksList.get(i)
            && data.chunkBlocksMeta[index] == replaceBlocksMetaList.get(i)) {
                if (y <= lavaMaxY) {
    ...
    public boolean isOceanBlock(WE_GeneratorData data, int index, int x, int y, int z) {
        if (data.chunkBlocks[index] != null) return data.chunkBlocks[index].getMaterial()
            .isLiquid();
        else return false;
    }
