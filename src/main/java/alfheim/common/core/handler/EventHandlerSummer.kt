...
object EventHandlerSummer {
	...
	fun onPlayerInteracted(e: PlayerInteractEvent) {
		...
		if (e.entityPlayer.heldItem?.let { it.item === AlfheimItems.eventResource && it.meta == EventResourcesMetas.LavaMelon } != true) return
	...
	fun onBlockBreak(e: BlockEvent.BreakEvent) {
		...
		if (e.block === Blocks.melon_block && e.world.rand.nextInt(10) == 0) {
			...
			if (word in 1..3 && !e.world.isRemote) ASJUtilities.say(e.player, "alfheimmisc.donteat.$word")
			
			e.isCanceled = true
			e.world.setBlockToAir(e.x, e.y, e.z)
			if (!e.world.isRemote) EntityRollingMelon(e.world).apply { setPosition(e.x + 0.5, e.y + 0.5, e.z + 0.5); onSpawnWithEgg(null) }.spawn()
		}
	}
}
