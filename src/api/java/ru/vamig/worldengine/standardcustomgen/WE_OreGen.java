...
public class WE_OreGen implements IWorldGenerator {
    ...
    public void add(Block ore, Block replace, int meta, int minVeinSize, int maxVeinSize, int minVeinsPerChunk,
        int maxVeinsPerChunk, int chanceToSpawn, int minY, int maxY) {
        oreGen.add(
            new WorldGenMinableParametrized(
                ore,
                replace,
                meta,
                minVeinSize,
        ...
        for (WorldGenMinableParametrized wgmp : oreGen) {
            ...
            for (int i = 0; i < veins; i++) {
                ...
                (new WorldGenMinable(
                    wgmp.ore,
                    wgmp.meta,
                    randInBounds(wgmp.minVeinSize, wgmp.maxVeinSize, random),
                    wgmp.replace)).generate(world, random, posX, posY, posZ);
            }
        }
    }
    ...
    public static class WorldGenMinableParametrized {
        ...
        public WorldGenMinableParametrized(Block ore, Block replace, int meta, int minVeinSize, int maxVeinSize,
