...
public class WE_RavineGen extends WE_CreateChunkGen {
    ...
    public void caveGen_func(WE_GeneratorData data, long rs, double gx, double gy, double gz, float fn1, float fn2,
        ...
        for (int k1 = 0; k1 < 256; ++k1) {
            if (k1 == 0 || r.nextInt(3) == 0) f5 = 1.0F + r.nextFloat() * r.nextFloat() * 1.0F;
            field_75046_d[k1] = f5 * f5;
        }

        for (; in1 < in2; ++in1) {
            double d12 = 1.5D + (double) MathHelper.sin((float) in1 * (float) Math.PI / (float) in2) * (double) fn1,
                d6 = d12 * dn;
            ...
            if (flag1 || r.nextInt(4) != 0) {
                ...
                if (gx >= d4 - 16.0D - d12 * 2.0D && gz >= d5 - 16.0D - d12 * 2.0D
                    && gx <= d4 + 16.0D + d12 * 2.0D
                    && gz <= d5 + 16.0D + d12 * 2.0D) {
                    int i4 = MathHelper.floor_double(gx - d12) - (int) data.chunk_X - 1,
                        l1 = MathHelper.floor_double(gx + d12) - (int) data.chunk_X + 1,
                        j4 = MathHelper.floor_double(gy - d6) - 1, i2 = MathHelper.floor_double(gy + d6) + 1,
                        k4 = MathHelper.floor_double(gz - d12) - (int) data.chunk_Z - 1,
                        j2 = MathHelper.floor_double(gz + d12) - (int) data.chunk_Z + 1;
                    ...
                    for (k2 = i4; !flag2 && k2 < l1; ++k2)
                        for (int l2 = k4; !flag2 && l2 < j2; ++l2) for (int i3 = i2 + 1; !flag2 && i3 >= j4 - 1; --i3) {
                            j3 = (k2 * 16 + l2) * 256 + i3;
                            if (i3 >= 0 && i3 < 256) {
                                if (isOceanBlock(data, j3, k2, i3, l2)) flag2 = true;
