...
public class WE_WorldTreeGen implements IWorldGenerator {
    ...
    public void add(Block wood, int woodMeta, Block leaves, int leavesMeta, Block sapling, Block vines, Block cocoa,
        int dispersion, int perChunk, int chanceForBig, int minHeight, boolean doVinesLog, boolean doVinesLeaves,
        byte ocp1, byte ocp2, byte ocp3, byte ocp4, byte ocp5, byte ocp6, int ts, int hl, int ldl, double ha, double bs,
        double sw, double ld) {
    ...
    public void add(Block bWood, int mWood, Block bLeaves, int mLeaves, Block bSapling, Block bVine, Block bCocoa,
        int v1, int v2, int minHeight, boolean vines) {
        smallTrees.add(new WE_TreeGen(false));
        bigTrees.add(null);
        // -//
        dispersions.add(v1);
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        ...
        for (int i = 0; i < smallTrees.size(); i++)
            if (random.nextInt(dispersions.get(i)) == 0) for (int i2 = 0; i2 < treesPerChunkCounts.get(i); i2++) {
                int x = chunkX * 16 + random.nextInt(16), z = chunkZ * 16 + random.nextInt(16),
                    y = world.getHeightValue(x, z);

                if (chanceForBigTree.get(i) <= 0) smallTrees.get(i)
                    .generate(world, random, x, y, z);
                else if (random.nextInt(chanceForBigTree.get(i)) == 0) bigTrees.get(i)
                    .generate(world, random, x, y, z);
                else smallTrees.get(i)
                    .generate(world, random, x, y, z);
            }
    }
}
