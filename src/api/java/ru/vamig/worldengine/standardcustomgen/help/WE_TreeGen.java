...
public class WE_TreeGen extends WorldGenerator {
    ...
    public Block bWood = Blocks.log, bLeaves = Blocks.leaves, bSapling = Blocks.sapling, bVine = Blocks.vine,
        bCocoa = Blocks.cocoa;
        ...
        if (y >= 1 && y + l + 1 <= 256) {
            ...
            for (int i1 = y; i1 <= y + 1 + l; ++i1) {
                ...
                for (int j1 = x - b0; j1 <= x + b0 && flag; ++j1)
                    for (k1 = z - b0; k1 <= z + b0 && flag; ++k1) if (i1 >= 0 && i1 < 256) {
                        if (!isReplaceable(world, j1, i1, k1)) flag = false;
                    } else flag = false;
            }
                ...
                if (isSoil && y < 256 - l - 1) {
                    ...
                    if (vinesGrowLeaves) {
                        for (k1 = y - 3 + l; k1 <= y + l; ++k1) {
                            ...
                            // -//
                            for (i2 = x - l1; i2 <= x + l1; ++i2)
                                for (j2 = z - l1; j2 <= z + l1; ++j2) if (world.getBlock(i2, k1, j2)
                                    .isLeaves(world, i2, k1, j2)) {
                                        if (random.nextInt(4) == 0 && world.getBlock(i2 - 1, k1, j2)
                                            .isAir(world, i2 - 1, k1, j2)) growVines(world, i2 - 1, k1, j2, 8);
                                        if (random.nextInt(4) == 0 && world.getBlock(i2 + 1, k1, j2)
                                            .isAir(world, i2 + 1, k1, j2)) growVines(world, i2 + 1, k1, j2, 2);
                                        if (random.nextInt(4) == 0 && world.getBlock(i2, k1, j2 - 1)
                                            .isAir(world, i2, k1, j2 - 1)) growVines(world, i2, k1, j2 - 1, 1);
                                        if (random.nextInt(4) == 0 && world.getBlock(i2, k1, j2 + 1)
                                            .isAir(world, i2, k1, j2 + 1)) growVines(world, i2, k1, j2 + 1, 4);
                                    }
                        }

                        if (bCocoa != null && random.nextInt(5) == 0 && l > 5)
                            for (k1 = 0; k1 < 2; ++k1) for (i3 = 0; i3 < 4; ++i3) if (random.nextInt(4 - k1) == 0) {
                                l1 = random.nextInt(3);
    ...
    public void growVines(World p_76529_1_, int p_76529_2_, int p_76529_3_, int p_76529_4_, int p_76529_5_) {
        ...
        while (true) {
            ...
            if (!p_76529_1_.getBlock(p_76529_2_, p_76529_3_, p_76529_4_)
                .isAir(p_76529_1_, p_76529_2_, p_76529_3_, p_76529_4_) || i1 <= 0) return;
            // -//
            setBlockAndNotifyAdequately(p_76529_1_, p_76529_2_, p_76529_3_, p_76529_4_, bVine, p_76529_5_);
            --i1;
        }
