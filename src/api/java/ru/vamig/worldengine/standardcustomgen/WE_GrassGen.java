...
public class WE_GrassGen implements IWorldGenerator {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        ...
        for (int i = 0; i < flowerList.size(); i++) if (random.nextInt(blocksForFlower.get(i)) <= 255)
            for (int i2 = 0; i2 < MathHelper.ceiling_float_int(256.0F / (float) blocksForFlower.get(i)); i2++) {
                ...
                if (waterGen.get(i)) {
                    if (world.getBlock(x, y - 1, z)
                        ...
                        .isLiquid()) {
                        --y;
                        while (true) if (world.getBlock(x, y - 1, z)
                            .getMaterial()
                            .isLiquid()) --y;
                        else {
                            if (world.getBlock(x, y - 1, z) == surface.get(i)
                                && world.getBlockMetadata(x, y - 1, z) == surfaceMeta.get(i))
                                world.setBlock(x, y, z, flowerList.get(i), flowerMetaList.get(i), 2);
                            break;
                        }
                    }
                } else if (world.getBlock(x, y - 1, z) == surface.get(i)
                    && world.getBlockMetadata(x, y - 1, z) == surfaceMeta.get(i))
                    world.setBlock(x, y, z, flowerList.get(i), flowerMetaList.get(i), 2);
            }
    }
