...
public class WE_ChunkProvider extends ChunkProviderGenerate {
    ...
    public Chunk provideChunk(int chunkX, int chunkZ) {
        ...
        for (WE_CreateChunkGen we_createChunkGen : createChunkGen_List) we_createChunkGen
            .gen(new WE_GeneratorData(this, chunkBlocks, chunkBlocksMeta, chunk_X, chunk_Z, chunkBiomes, 0, 0, 0));
        // -//
        for (int x = 0; x < 16; x++) for (int z = 0; z < 16; z++) {
            for (WE_CreateChunkGen_InXZ we_createChunkGen_inXZ : createChunkGen_InXZ_List) we_createChunkGen_inXZ
                .gen(new WE_GeneratorData(this, chunkBlocks, chunkBlocksMeta, chunk_X, chunk_Z, chunkBiomes, x, 0, z));
            for (int i = 0; i < chunkBiomes[x][z].createChunkGen_InXZ_List.size(); i++)
                chunkBiomes[x][z].createChunkGen_InXZ_List.get(i)
                    .gen(
                        new WE_GeneratorData(
                            this,
                            chunkBlocks,
                            chunkBlocksMeta,
                            chunk_X,
            ...
            for (int y = 255; y >= 0; y--) {
                ...
                for (int i = 0; i < chunkBiomes[x][z].createChunkGen_InXYZ_List.size(); i++)
                    chunkBiomes[x][z].createChunkGen_InXYZ_List.get(i)
                        .gen(
                            new WE_GeneratorData(
                                this,
                                chunkBlocks,
    ...
    public void populate(IChunkProvider chunkProvider, int chunkX, int chunkZ) {
        ...
        for (IWorldGenerator iWorldGenerator : decorateChunkGen_List)
            iWorldGenerator.generate(rand, chunkX, chunkZ, world, this, this);
        // -//
        WE_Biome b = WE_Biome.getBiomeAt(
            this,
            (long) chunkX * 16L + (long) rand.nextInt(16),
            (long) chunkZ * 16L + (long) rand.nextInt(16));
        for (int i = 0; i < b.decorateChunkGen_List.size(); i++) b.decorateChunkGen_List.get(i)
            .generate(rand, chunkX, chunkZ, world, this, this);
    ...
    public List getPossibleCreatures(EnumCreatureType type, int x, int y, int z) {
        ...
        return /*
                * type == EnumCreatureType.monster && this.scatteredFeatureGenerator.func_143030_a(x, y, z) ?
                * this.scatteredFeatureGenerator.getScatteredFeatureSpawnList() :
                */ b.getSpawnableList(type);
    }
    ...
    public Block genReturnBlock(Block[] chunkBlocks, int x, int y, int z) {
        if (x >= 0 && x <= 15 && y >= 0 && y <= 255 && z >= 0 && z <= 15) return chunkBlocks[(x * 16 + z) * 256 + y];
        else return null;
    }
