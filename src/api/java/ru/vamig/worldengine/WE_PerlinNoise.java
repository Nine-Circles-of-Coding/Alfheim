...
public class WE_PerlinNoise {
    ...
    public static double PerlinNoise2D(long seed, double x, double y, double persistence, int number_of_octaves) {
        ...
        for (int i = 1; i <= number_of_octaves; i++) {
            double frequency = Math.pow(2, i), amplitude = Math.pow(persistence, i);
    ...
    public static double CosineInterpolatedNoise2D(long seed, double x, double y) {
        ...
        double fx = Math.abs(x) - Math.abs(lx), fy = Math.abs(y) - Math.abs(ly);

        long x1 = Math.abs(x) == x ? lx + 1L : lx - 1L;
        long y1 = Math.abs(y) == y ? ly + 1L : ly - 1L;

        double v1 = SmoothNoise2D(seed, lx, ly), v2 = SmoothNoise2D(seed, x1, ly), v3 = SmoothNoise2D(seed, lx, y1),
            v4 = SmoothNoise2D(seed, x1, y1);

        double i1 = CosineInterpolate(v1, v2, fx), i2 = CosineInterpolate(v3, v4, fx);

        return CosineInterpolate(i1, i2, fy);
    }

    public static double SmoothNoise2D(long seed, long x, long y) {
        double corners = (NumberNoise2D(seed, x - 1L, y - 1L) + NumberNoise2D(seed, x + 1L, y - 1L)
            + NumberNoise2D(seed, x - 1L, y + 1L)
            + NumberNoise2D(seed, x + 1L, y + 1L)) / 16L,
            sides = (NumberNoise2D(seed, x - 1L, y) + NumberNoise2D(seed, x + 1L, y)
                + NumberNoise2D(seed, x, y - 1L)
                + NumberNoise2D(seed, x, y + 1L)) / 8L,
