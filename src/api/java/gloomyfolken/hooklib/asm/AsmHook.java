...
public class AsmHook implements Cloneable, Comparable<AsmHook> {
    ...
    protected void createMethod(HookInjectorClassVisitor classVisitor, boolean isStatic, boolean isAbstract,
        String superClass) {
                    ...
                    if ((superClassInfoList.length & 1) == 0) {
                        for (int i = 0; i < superClassInfoList.length - 2; i += 2) {
                            if (i + 2 != superClassInfoList.length - 1) {
                                inj.visitVarInsn(
                                    Integer.parseInt(superClassInfoList[i + 1]),
                                    Integer.parseInt(superClassInfoList[i + 2]));
                            } else {
                                throw new IllegalArgumentException("? superClass is not correct ?"); // ?никогда не
                                                                                                     // сработает?
                            }
                        }
                        inj.visitMethodInsn(
                            INVOKESPECIAL,
                            superClassInfoList[0],
                            "<init>",
                            superClassInfoList[superClassInfoList.length - 1],
                            false);
                    } else {
    ...
    public String toString() {
        ...
        sb.append(", ReturnValue=")
            .append(returnValue);
        if (returnValue == ReturnValue.PRIMITIVE_CONSTANT) sb.append(", Constant=")
            .append(primitiveConstant);
        sb.append(", InjectorFactory: ")
            .append(
                injectorFactory.getClass()
                    .getName());
    ...
    public class Builder extends AsmHook {
         ...
         *
         * @throws IllegalStateException    если returnCondition == NEVER (т. е. если setReturnCondition() не
         *                                  вызывался).
         ...
         *                                  целевого метода указан как void (или setTargetMethodReturnType ещё не
         *                                  вызывался).
         *                                  Нет смысла использовать значение, которое вернул хук-метод, если метод
         *                                  возвращает void.
         */
