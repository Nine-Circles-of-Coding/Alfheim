...
class SubTileLightning: SubTileAnomalyBase() {
	...
	override fun performEffect(target: Any) {
		...
		val dmg = (Math.random() * 2 + 3) / vt.set(x + 0.5, y + 0.5, z + 0.5).add(-target.posX, -target.posY, -target.posZ).mag() / 2.0
		val damaged = target.attackEntityFrom(DamageSourceSpell.anomaly, min(dmg, 2.5).F * 4f)
		
		if (damaged && target is EntityPlayerMP && !target.worldObj.isRemote) RagnarokHandler.addMadness(target)
