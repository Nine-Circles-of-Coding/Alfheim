...
//					8..10
	...
	override fun attackEntityFrom(src: DamageSource?, amount: Float): Boolean {
		...
		val cout = min(4, PYLONS.sumOf { min(1, worldObj.getEntitiesWithinAABB(EntityFireSpirit::class.java, getBoundingBox(x + it[0] + 0.5, y + it[1] + if (master) 0.5 else -0.5, z + it[2] + 0.5).expand(0.5)).size) })
