...
public class WE_BigTreeGen extends WorldGenerator {
    ...
    public boolean generate(World p_76484_1_, Random p_76484_2_, int p_76484_3_, int p_76484_4_, int p_76484_5_) {
        ...
        int[] _aint = new int[] { basePos[0], basePos[1], basePos[2] },
        ...
        if (!block.canSustainPlant(
            ...
            basePos[2],
            ForgeDirection.UP,
            (BlockSapling) bSapling)) br = false;
        else {
            int i = checkBlockLine(p_76484_1_, _aint, _aint1);
            if (i == -1) br = true;
            else if (i < 6) br = false;
            else {
                heightLimit = i;
                br = true;
            }
        ...
        if (!br) return false;
            ...
            while (i1 >= 0) {
                ...
                // -//
                float f2;
                if ((double) i1 < (double) heightLimit * 0.3D) f2 = -1.618F;
                else {
                    float f = (float) heightLimit / 2.0F, f1 = (float) heightLimit / 2.0F - (float) i1;
                    if (f1 == 0.0F) f2 = f;
                    else if (Math.abs(f1) >= f) f2 = 0.0F;
                    else f2 = (float) Math.sqrt(Math.pow(Math.abs(f), 2.0D) - Math.pow(Math.abs(f1), 2.0D));
                    f2 *= 0.5F;
                }
                // -//
                if (!(f2 < 0.0F)) {
                    for (double d0 = 0.5D; j1 < i; ++j1) {
                        double d1 = scaleWidth * (double) f2 * ((double) p_76484_2_.nextFloat() + 0.328D),
                            d2 = (double) p_76484_2_.nextFloat() * 2.0D * Math.PI;
                        ...
                        if (checkBlockLine(p_76484_1_, aint1, aint2) == -1) {
                            ...
                            double d3 = Math.sqrt(
                                Math.pow(Math.abs(basePos[0] - aint1[0]), 2.0D)
                                    + Math.pow(Math.abs(basePos[2] - aint1[2]), 2.0D)),
                                d4 = d3 * branchSlope;

                            if ((double) aint1[1] - d4 > (double) l) aint3[1] = l;
                            else aint3[1] = (int) ((double) aint1[1] - d4);
            ...
            for (int j_1 = leafNodes.length; i_1 < j_1; ++i_1) {
                ...
                for (int i1_2 = leafNodes[i_1][1] + leafDistanceLimit; l_2 < i1_2; ++l_2) {
                    float p1 = l_2 - leafNodes[i_1][1],
                        p2 = p1 >= 0 && p1 < leafDistanceLimit ? (p1 != 0 && p1 != leafDistanceLimit - 1 ? 3.0F : 2.0F)
                            : -1.0F;
                    byte b1_3 = otherCoordPairs[1], b2_3 = otherCoordPairs[4];
                    int[] aint_3 = new int[] { leafNodes[i_1][0], l_2, leafNodes[i_1][2] },
                        aint1_3 = new int[] { 0, 0, 0 };
                    ...
                    for (aint1_3[1] = aint_3[1]; i1_3 <= l_3; ++i1_3) {
                        ...
                        j1_3 = -l_3;
                        while (j1_3 <= l_3) {
                            double d0_3 = Math.pow((double) Math.abs(i1_3) + 0.5D, 2.0D)
                                + Math.pow((double) Math.abs(j1_3) + 0.5D, 2.0D);
                            if (d0_3 > (double) (p2 * p2)) ++j1_3;
                            else {
                                ...
                                if (!block1.isAir(p_76484_1_, aint1_3[0], aint1_3[1], aint1_3[2])
                                    && !block1.isLeaves(p_76484_1_, aint1_3[0], aint1_3[1], aint1_3[2])) ++j1_3;
                                else {
            ...
            int[] aint_4 = new int[] { basePos[0], basePos[1], basePos[2] },
            ...
            for (int[] aint_5 = new int[] { basePos[0], basePos[1], basePos[2] }; i_5 < leafNodes.length; ++i_5) {
                ...
                if ((double) k_5 >= (double) heightLimit * 0.2D) func_150530_a(p_76484_1_, aint_5, aint2_5, bWood);
            }
            // -//
            return true;
        }
    ...
    public int checkBlockLine(World worldObj, int[] p_76496_1_, int[] p_76496_2_) {
        ...
        for (b1 = 0; b0 < 3; ++b0) {
            ...
            if (Math.abs(aint2[b0]) > Math.abs(aint2[b1])) b1 = b0;
        }
        ...
        if (aint2[b1] == 0) return -1;
            ...
            for (j = aint2[b1] + b4; i != j; i += b4) {
                ...
                if (!isReplaceable(worldObj, aint3[0], aint3[1], aint3[2])) break;
            }
            // -//
    ...
    public void func_150530_a(World worldObj, int[] p_150530_1_, int[] p_150530_2_, Block p_150530_3_) {
        ...
        if (aint2[b1] != 0) {
            ...
            for (int j = aint2[b1] + b4; i != j; i += b4) {
                ...
                int k = Math.abs(aint3[0] - p_150530_1_[0]), l = Math.abs(aint3[2] - p_150530_1_[2]),
                    i1 = Math.max(k, l);

                if (i1 > 0) if (k == i1) b5 += 4;
                else if (l == i1) b5 += 8;
    ...
    public boolean cb(Block b) {
        for (Block block : ab) if (b == block) return true;
        for (Material material : am) if (b.getMaterial() == material) return true;
        return false;
    }
}
